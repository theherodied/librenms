...
```
Specify the location of the collectd rrd files. Note that the location in config.php should be consistent with the location set in /etc/collectd.d/rrdtool.conf and etc/collectd.d/rrdtool.conf

```php
<Plugin rrdtool>
        #DataDir "/opt/librenms/rrd/collectd"
